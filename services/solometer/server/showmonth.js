...
{
  var i,j,k,jahr,monat,tag,monat_akt,monat_akt_setzen=true;
  var fjahr, fmonat;
  ...
  for(i=0;i<30;i++)
    tageswerte[i] = 0;
  // Check for GET request params
  if((mdate = get_GET_param("MONAT"))) {
    Erg3 = mdate.match(/[0-9]+/g);
    if(Erg3 && Erg3.length == 2) {
       fmonat = parseInt(Erg3[0],10) - 1;
       fjahr = parseInt(Erg3[1],10);
    } else {
      fjahr = 0;
      fmonat = 0;
    }
  ...
  i = 0;
  tmparr3 = str.split('\n');
  for(k=0;k<tmparr3.length && i < 31;k++) {
    // Suche 13 Int 0-2 = Datum
    Erg3 = tmparr3[k].match(/[0-9]+/g);
	...
	tagesnamen[30-i] = tag+'.'+mnamenlookup[monat];
	j = i;
	i++;
      }
    }
  ...
  monatstitel = lmnamenlookup[fmonat]+" 20"+fjahr;
  tmpstr = num2kstr(summe);
  m_wboxtxt1 = lmnamenlookup[fmonat]+": "+tmpstr[0]+" "+tmpstr[1]+"Wh";
...
function read_mins_data()
{
  ...
  if((mdate = get_GET_param("TAG")) == false) {
	...
	// Caching umgehen...Ok, so ist's suboptimal.
	  dateiname='day'+mdate+'.dat?'+Math.random();
  }
  req.open('GET', dateiname, true);
...
function read_meas_data()
{
  ...
  if((mdate = get_GET_param("TAG")) == false) {
	...
	// Caching umgehen...Ok, so ist's suboptimal.
	  dateiname='meas'+mdate+'.dat?'+Math.random();
  }
  // Caching umgehen...Ok, so ist's suboptimal.
  //dateiname='meas.dat?'+Math.random();
  req.open('GET', dateiname, true);
